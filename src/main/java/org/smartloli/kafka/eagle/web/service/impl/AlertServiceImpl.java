...
public class AlertServiceImpl implements AlertService {
	...
	public String getAlertTypeList() {
		...
		for (String type : KafkaConstants.TYPE) {
			JSONObject object = new JSONObject();
	...
	public boolean findAlarmConfigByGroupName(Map<String, Object> params) {
		return alertDao.findAlarmConfigByGroupName(params) > 0;
	}
	...
	public String getAlertClusterTypeList(String type, Map<String, Object> params) {
		...
		if ("type".equals(type)) {
			if (params.get("search").toString().length() > 0) {
				for (String cluster : KafkaConstants.CLUSTER) {
			...
			if (params.get("search").toString().length() > 0) {
				for (String level : KafkaConstants.LEVEL) {
			...
			if (params.get("search").toString().length() > 0) {
				for (int maxtimes : KafkaConstants.MAXTIMES) {
