...
import org.smartloli.kafka.eagle.web.protocol.OffsetZkInfo;
...
public class OffsetServiceImpl implements OffsetService {
	...
	private List<OffsetInfo> getLogSize(String clusterAlias, String topic, String group) {
		...
		for (String partition : partitions) {
            ...
            offsetInfo.setPartition(partitionInt);
            offsetInfo.setLogSize(logSize);
            offsetInfo.setCreate(offsetZk.getCreate());
            offsetInfo.setModify(offsetZk.getModify());
            offsetInfo.setOffset(offsetZk.getOffset());
            offsetInfo.setLag(offsetZk.getOffset() == -1 ? 0 : logSize - offsetZk.getOffset());
			offsetInfo.setOwner(offsetZk.getOwners());
	...
	public String getOffsetsGraph(Map<String, Object> params) {
		...
		if (topicOffsets.size() > 0) {
			for (BScreenConsumerInfo topicOffset : topicOffsets) {
                lagX.add(DateUtils.convertUnixTime(topicOffset.getTimespan(), "yyyy-MM-dd HH:mm"));
                lagY.add(topicOffset.getLag());
                producerX.add(DateUtils.convertUnixTime(topicOffset.getTimespan(), "yyyy-MM-dd HH:mm"));
                producerY.add(topicOffset.getDifflogsize());
                consumerX.add(DateUtils.convertUnixTime(topicOffset.getTimespan(), "yyyy-MM-dd HH:mm"));
	...
	private List<OffsetInfo> getKafkaLogSize(TopicOffsetInfo topicOffset) {
		...
		if (tps != null && partitionOffset != null) {
			for (Entry<TopicPartition, Long> entrySet : tps.entrySet()) {
                ...
                offsetInfo.setCreate(DateUtils.getCurrentDate());
                offsetInfo.setModify(DateUtils.getCurrentDate());
