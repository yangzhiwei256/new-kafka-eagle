...
public class BrokerServiceImpl implements BrokerService {
	...
	public List<String> topicList(String clusterAlias) {
        ...
        if (kafkaClustersConfig.getClusterConfigByName(clusterAlias).getSasl().getCgroupEnable()) {
            topics = Arrays.asList(kafkaClustersConfig.getClusterConfigByName(clusterAlias).getSasl().getCgroupTopics().split(","));
        } else {
            KafkaZkClient zkc = KafkaResourcePoolUtils.getZookeeperClient(clusterAlias);
            try {
	...
	public Map<String, Object> createTopicPartitions(String clusterAlias, String topic, int totalCount) {
        ...
        try {
            ...
            log.info("Add kafka topic partitions has error", e);
